...
<meta name="keywords" content="<?php echo $row_info["meta_k"]?>;" />
<meta name="description" content="<?php echo $row_info["meta_d"]?>;" />
...
<noscript>
<meta http-equiv="refresh" content="0; url=page_nosrc.php?lang=<?php echo $lang; ?>&city=<?php echo $_GET['city']; ?>&b_def=<?php echo $_GET['b_def']; ?>&defcat=<?php echo $_GET['defcat']; ?>&link=<?php echo $_GET['link']; ?>" /></noscript>
</head>
<body>
...
//proverka na sootvetstvie firmi.....................	
	if(mysql_num_rows($result_ltd)>0)
	{
		...
		$result_stats = mysql_query("SELECT login,sh_name,views FROM stats WHERE login = '".$row_info['login']."' AND sh_name = '".$sh_name."'");
		$row_stats = mysql_fetch_array($result_stats);
		if(mysql_num_rows($result_stats)>0)
		{
			$views = $row_stats['views'] + 1;
			mysql_query("UPDATE stats SET value = NOW(), views = '$views' WHERE login = '".$row_info['login']."' AND sh_name = '".$sh_name."'");
		}
		else
		{
			$views = '1';
			mysql_query("INSERT INTO stats VALUES ('".$row_info['login']."','".$sh_name."',NOW(),'$views')");
		}
		
		$ip_top = ($_SERVER['HTTP_X_FORWARDED_FOR'] == "" ? $_SERVER['REMOTE_ADDR'] : $_SERVER['HTTP_X_FORWARDED_FOR']);
		$result_top_index = mysql_query("SELECT * FROM top_index WHERE sh_name = '".$sh_name."' AND ip = '$ip_top'");
		$row_top_index = mysql_fetch_array($result_top_index);
		if(mysql_num_rows($result_top_index)>0)
		{
			$views_top = $row_top_index['votes'] + 1;
			mysql_query("UPDATE top_index SET date_reg = NOW(), votes = '$views_top' WHERE ip = '$ip_top' AND sh_name = '$sh_name' AND UNIX_TIMESTAMP() - UNIX_TIMESTAMP(date_reg) > 300");
		}
		else
		{
			$views_top = '1';
			mysql_query("INSERT INTO top_index VALUES ('$sh_name',NOW(),'$ip_top','$views_top')");
		}
	}
	else
	{
    ...
    <?php
		...
		if(mysql_num_rows($result_comm)>0)
		{
			...
			{		
				...
				<tr>
				<td bordercolor="#CCC" width="90px" align="left" valign="top"><img style="border:1px solid #CCC;" src="users/<?php echo $row_comm['avatar']; ?>" alt="<?php echo $row_comm['nick']; ?>" /></td>
