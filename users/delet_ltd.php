...
<title><?php echo $delet_ltd_title; ?></title>
...
{               	
	...
	{	
		...
		if($_GET['proof'] == 'yes')
		{	
			$result_check_gallery = mysql_query("SELECT login,sh_name FROM gallery WHERE login = '$login' AND sh_name = '$sh_name'");
			if(mysql_num_rows($result_check_gallery)>0)
			{
				$result_delet_gallery = mysql_query("DELETE FROM gallery WHERE login = '$login' AND sh_name = '$sh_name'");			
			}
			...
			$result_delet_ltd_rus = mysql_query("DELETE FROM ltd_page_rus WHERE login = '$login' AND sh_name = '$sh_name'");
			$result_delet_ltd_lat = mysql_query("DELETE FROM ltd_page_lat WHERE login = '$login' AND sh_name = '$sh_name'");
			
			if(!$result_delet_ltd_rus && !$result_delet_ltd_lat)
			{
			...
			{
				$result_check_cat_rus = mysql_query("SELECT ltd_name FROM catalogue_rus WHERE ltd_name = '$sh_name'");
				$result_check_cat_lat = mysql_query("SELECT ltd_name FROM catalogue_lat WHERE ltd_name = '$sh_name'");
				if(mysql_num_rows($result_check_cat_rus)>0 && mysql_num_rows($result_check_cat_lat)>0)
				{
					mysql_query("DELETE FROM catalogue_rus WHERE ltd_name = '$sh_name' AND login = '$login'");
					mysql_query("DELETE FROM catalogue_lat WHERE ltd_name = '$sh_name' AND login = '$login'");
				}
				
				$result_check_mess = mysql_query("SELECT for_sh_name FROM messages WHERE for_sh_name = '$sh_name' AND login = '$login'");
				if(mysql_num_rows($result_check_mess)>0)
				{
					mysql_query("DELETE FROM messages WHERE for_sh_name = '$sh_name' AND login = '$login'");
				}
				
				$result_check_stats = mysql_query("SELECT sh_name FROM stats WHERE sh_name = '$sh_name'");
				if(mysql_num_rows($result_check_stats)>0)
				{
					mysql_query("DELETE FROM stats WHERE sh_name = '$sh_name' AND login = '$login'");
				}
				
				message_succes($lang,"<p align='center' class='join_us_error'>".$delet_ltd_succ_1.$sh_name.$delet_ltd_succ_2."</p><p><a href='profile.php?lang=".$lang."'>".$change_user_nothing_link."</a></p>
							   <meta http-equiv='Refresh' content='40; URL=profile.php?lang=".$lang."'>");
			}
		}
        ...
        <tr>
        <td align="center" colspan="2"><p class="join_us_error"><?php echo $delet_warning." ".$sh_name."?"; ?></p></td>
        </tr>
        <tr>
        <td><a style="padding-left:120px; text-decoration:none;" href='delet_ltd.php?lang=<?php echo $lang;?>&ltd=<?php echo $sh_name; ?>&proof=yes'><?php echo $delet_yes; ?></a></td>
