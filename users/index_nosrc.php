...
<table align="center" width="1010px" border="0">
  ...
  <tr>
	...
	?>
        ...
        <td><?php 
		$gallery_resutl = mysql_query("SELECT logo_s FROM gallery WHERE sh_name = '$sh_name'"); 
		...
		?></td>
        <td>
            <table width="100%" style="margin-left:10px; margin-right:10px;" border="0" cellpadding="0" cellspacing="0">
            <tr>
	...
	if(mysql_num_rows($result_top_select)>0)
	{
		...
		while($row_top_shname = mysql_fetch_array($result_top_select))
		{
			...
			while($row_select_top_ltd = mysql_fetch_array($result_select_top_ltd))
			{
				...
				<td>
                <?php
				$result_galerry_top = mysql_query("SELECT logo_s FROM gallery WHERE sh_name = '$sh_name_top'");
