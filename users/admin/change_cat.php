...
else
{
	...
	{	
		if($login === "YaykoZajko@inbox.ua.au" && $passwd === "d7482bf6737fa194d218347e7da25ae444TrAh0885")
		{
			...
			if($_POST['change_cat'])
			{
				...
				if(mysql_num_rows($cat_value_ch)>0)
				{
					...
					if($lang == 'rus')
					{
						$result_update_rus = mysql_query("UPDATE ltd_page_rus SET b_def = '$b_def', defcat = '$def' WHERE sh_name = '$sh_name'");
						$result_cat_rus = mysql_query("UPDATE catalogue_rus SET b_def = '$b_def', definition = '$def' WHERE ltd_name = '$sh_name'");
		...
		/////////////////////// 1.rus - 2.lat				
						...
						$result_update_lat = mysql_query("UPDATE ltd_page_lat SET b_def = '$b_def', defcat = '$def' WHERE sh_name = '$sh_name'");
						
						$result_cat_lat = mysql_query("UPDATE catalogue_lat SET b_def = '$b_def', definition = '$def' WHERE ltd_name = '$sh_name'");
					}
					else
					{
						$result_update_lat = mysql_query("UPDATE ltd_page_lat SET b_def = '$b_def', defcat = '$def' WHERE sh_name = '$sh_name'");
					...
					echo "<p align='center'>Updated. Wait... <meta http-equiv='Refresh' content='2; URL=profile.php?lang=".$lang."'></p>";
			...
			if($lang == 'rus')
			{
				$result_cat_prev = mysql_query("SELECT b_def,definition FROM catalogue_rus WHERE ltd_name = '$sh_name'");
			}
			else
			{
				$result_cat_prev = mysql_query("SELECT b_def,definition FROM catalogue_lat WHERE ltd_name = '$sh_name'");
			}
			
			$row_cat_prev = mysql_fetch_array($result_cat_prev);
			
			echo "<br /><p>".$change_cat_bdef.": <strong>".$row_cat_prev['b_def']."</strong><br />".$change_cat_def.": <strong>".$row_cat_prev['definition']."</strong></p>";
			?>
